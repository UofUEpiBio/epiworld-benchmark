...
prepare_data_sets <- function(datafile = "calibration/sir.rds", train_fraction = 0.7, ndays = 50) {
  ...
  train_x <- arrays_1d[id_train,1,, drop = FALSE]
  train_x <- aperm(train_x, c(1, 3, 2))
  ...
  test_x <- arrays_1d[id_test,1,, drop = FALSE]
  test_x <- aperm(test_x, c(1, 3, 2))
...
# Function to build and train the LSTM model with different hyperparameters
build_and_train_model <- function(train, test, theta, N_train, ndays, units = units, batch_size = batch_size, epochs = 10, seed = 331) {
  # Build the LSTM model
  model <- keras_model_sequential() %>%
    layer_lstm(
      units = units,
  ...
  history <- model %>% fit(
    ...
    validation_split = 0.2,
    verbose = 0  # Suppress output for parallelization
  )
...
# Function to search for the best model parameters in parallel
tune_lstm_hyperparameters <- function(train, test, theta, N_train, ndays, seed = 331) {
  ...
  results <- parallel::mclapply(1:nrow(param_grid), function(i) {
    ...
    build_and_train_model(
      train, test, theta, N_train, ndays,
      units = params$units,
...
find_best_model <- function(results) {
  MAEs <- sapply(results, function(res) mean(res$MAEs))
  best_idx <- which.min(MAEs)
  best_model <- results[[best_idx]]
  
  print(sprintf("Best model found at index %d with MAE: %.4f", best_idx, MAEs[best_idx]))
  return(best_model)
}

# Visualize the best model's results
visualize_best_model <- function(best_model, test, theta, MAEs, N, N_train) {
  ...
  p2 <- ggplot(alldat_wide, aes(x = Observed, y = Predicted)) +
    ...
    labs(
      ...
      caption  = "Predictions made using an LSTM with tuned parameters."
    )
  print(p2)
}

# Main function to run everything
main_with_hyperparameter_tuning <- function() {
  # Simulate data
  simulate_data()
  ...
  results <- tune_lstm_hyperparameters(train, test, theta, N_train, ndays)
  
  # Find the best model based on MAE
  best_model <- find_best_model(results)
  
  # Print the MAE of the best model
  cat("Best model MAEs:", best_model$MAEs, "\n")
  
  # Visualize results for the best model
  visualize_best_model(best_model, test, theta, best_model$MAEs, N, N_train)
}

# Run the main function with hyperparameter tuning
main_with_hyperparameter_tuning()
