...
```{r}
experiment_cnn_models <- function(train_data, test_data, theta, input_shape, output_units, epochs = 50) {
    ...
    for (kernel_size in kernel_sizes) {
      for (activation in activations) {
        for (dense in dense_units) {
          ...
          model <- build_cnn_model(input_shape = input_shape, output_units = output_units,
                                   filters = filters, kernel_size = kernel_size,
                                   activation = activation, dense_units = dense)
...
main_pipeline <- function(N, n, ndays, ncores, best_filters, best_kernel_size, best_activation, best_dense_units, epochs = 50) {
...
# Define the best hyperparameters from your experiments
best_filters <- 32
best_kernel_size <- c(5, 5)
best_activation <- "relu"
best_dense_units <- 64

# Run the main_pipeline function with the best hyperparameters
main_pipeline(
  N = 20000, 
  n = 5000, 
  ndays = 50, 
  ...
  best_kernel_size = best_kernel_size, 
