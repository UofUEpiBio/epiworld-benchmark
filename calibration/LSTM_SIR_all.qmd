...
# Function to find the best model based on the lowest MAE
...
# Evaluate the model and return the Mean Absolute Error (MAE)
# Evaluate the model and return the Mean Absolute Error (MAE)
...
# Function to visualize LSTM model results
plot_results_lstm <- function(pred, test_data, theta, MAEs) {
  # Prepare predicted data for plotting
  pred[, id := 1L:.N]
  pred[, crate := qlogis(crate) * 10]  # Applying logit transformation to 'crate'
  pred_long <- melt(pred, id.vars = "id")

  # Prepare observed (test) data for plotting
  theta_long <- as.data.table(test_data$y)
  setnames(theta_long, colnames(theta))  # Ensure column names match theta
  theta_long[, id := 1L:.N]
  theta_long[, crate := qlogis(crate) * 10]  # Applying logit transformation to 'crate'
  theta_long <- melt(theta_long, id.vars = "id")

  # Combine predicted and observed data for plotting
  alldat <- rbind(
    cbind(pred_long, Type = "Predicted"),
    cbind(theta_long, Type = "Observed")
  )

  # Boxplot: Predicted vs Observed
  ...
  # Reshape data for scatter plot: Observed vs Predicted
  alldat_wide <- dcast(alldat, id + variable ~ Type, value.var = "value")

  # Assign proper names to the variables for clarity
  vnames <- data.table(
    variable = c("preval", "crate", "ptran", "prec"),
    Name     = paste(
  ...
  p2 <- ggplot(alldat_wide, aes(x = Observed, y = Predicted)) +
    facet_wrap(~ Name, scales = "free") +
    geom_abline(slope = 1, intercept = 0) +  # Add y=x reference line
    geom_point(alpha = .2) +
    labs(
      title = "Observed vs Predicted (Validation Set)",
      subtitle = sprintf(
