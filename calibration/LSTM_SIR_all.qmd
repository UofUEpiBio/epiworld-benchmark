...
evaluate_model_lstm <- function(model, test_data) {
  pred <- predict(model, x = test_data$x) |> as.data.table() |> setnames(colnames(test_data$y))
...
# Visualizing results
plot_results_lstm <- function(pred, test_data, theta, MAEs) {
  ...
  p2 <- ggplot(alldat_wide, aes(x = Observed, y = Predicted)) +
    ...
    labs(
      title    = "Observed vs Predicted (validation set)",
      subtitle = sprintf(
        "The model includes %i simulated datasets.",
        nrow(test_data$y)
      ),
      caption  = "Predictions made using an LSTM model."
    )
  
  print(p2)
}

# Run the best model and generate predictions
best_model <- best_model_result$model
pred <- predict(best_model, x = test$x) |> as.data.table() |> setnames(colnames(test$y))
MAEs <- abs(pred - as.matrix(test$y)) |> colMeans()

# Plot the results
plot_results_lstm(pred, test, theta2, MAEs)
```
